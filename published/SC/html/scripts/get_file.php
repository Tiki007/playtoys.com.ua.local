...
//        $getFileParam = Crypt::FileParamDeCrypt($_GET["getFileParam"]);
    //echo $getFileParam;
...
//            {
            ...
            $fileToDownLoad = 
                DIR_TEMP . '/' . date('Y-m-d') . '_' . ((int)(date('H') / 4)) . '_disc_' 
                . $cs_skidka . '_' . $cs_skidka_ua . '_yandex.xml'
            ;
...
//    else if ($res == 0 && strlen($fileToDownLoad) > 0 && file_exists($fileToDownLoad) && $direct_mode) {
    ...
    if ($res == 0 && strlen($fileToDownLoad) > 0 && file_exists($fileToDownLoad) && $direct_mode) {
