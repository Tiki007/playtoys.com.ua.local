<?php
    ...
    class YandexMarket extends Module
    {
        ...
        function _exportBegin($f)
        {
            ...
            fwrite($f, "				<currencies>\n");
            fwrite($f, "					<currency id=\"RUR\" rate=\"1\"/>\n");
            fwrite($f, " 					<currency id=\"USD\" rate=\"CBRF\"/>\n");
            fwrite($f, " 					<currency id=\"EUR\" rate=\"CBRF\"/>\n");
            fwrite($f, " 					<currency id=\"UAH\" rate=\"CBRF\"/>\n");
            fwrite($f, "				</currencies>\n");
        }
        ...
        function _exportProducts($f, $rate, $export_product_name, &$_ProductIDs)
        {
            ...
            for (; $iter < $proCount; $iter += 100) {
                ...
                while ($product = db_fetch_row($q)) {
                    ...
                    fwrite($f, "					<offer available=\"".(($product['in_stock'] || !CONF_CHECKSTOCK)?'true':'false')."\" id=\"".$product["productID"]."\">\n");
                    fwrite($f, "						<url>".str_replace('&', '&amp;', set_query('ukey=product&furl_enable=1&product_slug='.$product['slug'].'&productID='.$product['productID'].'&from=ya', $store_url))."</url>\n");
                    fwrite($f, "						<price>".RoundFloatValueStr($product["Price"] * $rate)."</price>\n");
                    ...
                    if ($pic_row) {
                        if (strlen($pic_row["filename"]) && file_exists(DIR_PRODUCTS_PICTURES."/".$pic_row["filename"]))
                            ...
                            if (strlen($pic_row["thumbnail"]) && file_exists(DIR_PRODUCTS_PICTURES."/".$pic_row["thumbnail"]))
                                fwrite($f, "						<picture>".$picture_url.str_replace(' ', '%20', $this->_deleteHTML_Elements($pic_row["thumbnail"]))."</picture>\n");
                    }
