<?php
    ...
    class Market extends Module
    {
        ...
        private $_url_path = '/published/SC/html/scripts/get_file.php?getFileParam=R2V0WWFuZGV4&did=73';
...
//            $_XXX = 'var';
            ...
            if (isset($_POST['market_export_local_delivery_cost_enabled']) && $_POST['market_export_local_delivery_cost_enabled']) {
                fwrite($f, "				<local_delivery_cost>".$local_delivery_cost."</local_delivery_cost>\n");
            }
        ...
        function _exportBegin($f)
        {
            ...
            fwrite($f, "	<!DOCTYPE yml_catalog SYSTEM \"shops.dtd\">\n");
            fwrite($f, "		<yml_catalog date=\"".date("Y-m-d H:i")."\">\n");
            fwrite($f, "			<shop>\n");
            fwrite($f, "				<name>".$this->_deleteHTML_Elements(CONF_SHOP_NAME)."</name>\n");
            fwrite($f, "				<company>".$this->_deleteHTML_Elements(CONF_SHOP_NAME)."</company>\n");
            fwrite($f, "				<url>".$this->getStoreUrl()."</url>\n");
            fwrite($f, "				<currencies>\n");
            fwrite($f, "					<currency id=\"UAH\" rate=\"1\"/>\n");
            fwrite($f, "					<currency id=\"USD\" rate=\"CB\"/>\n");
            fwrite($f, "					<currency id=\"EUR\" rate=\"CB\"/>\n");
            fwrite($f, "				</currencies>\n");
        }
...
//            $_XXX = 'var';
            ...
            while ($row = db_fetch_row($q)) {
                ...
                if ($row[2] <= 1) {
                    fwrite($f, "					<category id=\"".$row[0]."\">".$row[1].
                        "</category>\n");
                } else {
                    fwrite($f, "					<category id=\"".$row[0]."\" parentId=\"".$row[2]."\">".$row[1]."</category>\n");
                }
            }
...
//                $picture_url = (MOD_REWRITE_SUPPORT&&false)?$store_url.'products_pictures/':BASE_URL.URL_PRODUCTS_PICTURES.'/';
                ...
                while ($product = db_fetch_row($q)) {
                    ...
                    fwrite($f, "					<offer available=\"".(($product['zakaz'])?'':'true')."\" id=\"".$product["productID"]."\">\n");
                    fwrite($f, "						<url>".str_replace('&', '&amp;', set_query('ukey=product'.(MOD_REWRITE_SUPPORT?'&furl_enable=1':'').'&product_slug='.$product['slug'].'&productID='.$product['productID'], $store_url))."</url>\n");
                    fwrite($f, "						<price>".RoundFloatValueStr(priceDiscount($product["Price"], $product["skidka"], $product["ukraine"]) * $rate)."</price>\n");
                    fwrite($f, "						<currencyId>UAH</currencyId>\n");
                    fwrite($f, "						<categoryId>".$product["categoryID"]."</categoryId>\n");
...
//                            fwrite($f, "						<picture>".$picture_url.str_replace(' ', '%20', $this->_deleteHTML_Elements($pic_row["filename"]))."</picture>\n");
//                        else
//                            if (strlen($pic_row["filename"]) && file_exists(DIR_PRODUCTS_PICTURES."/".$pic_row["enlarged"]))
//                                fwrite($f, "						<picture>".$store_url.URL_PRODUCTS_PICTURES.'/'.str_replace(' ', '%20', $this->_deleteHTML_Elements($pic_row["enlarged"]))."</picture>\n");
//                    }
                        if ($pic_row) {
...
//				fwrite( $f, "                        <delivery>true</delivery>\n" );
                    if ($local_delivery_cost_enabled && ($product['free_shipping'] || $product['shipping_freight'])) {
                        ...
                        fwrite($f, "						<local_delivery_cost>".($product['free_shipping']?'0':(float)$product['shipping_freight'])."</local_delivery_cost>\n");
                    }
                    
                    $product["name"] = $this->_deleteHTML_Elements($_NameAddi.$product["name"]);
                    
                    fwrite($f, "						<name>".$product["name"]."</name>\n");
                    
                    $product["product_code"] = $product["product_code"] = $this->_deleteHTML_Elements($product["product_code"]);
                    
                    fwrite($f, "						<vendorCode>".$product["product_code"]."</vendorCode>\n");
                    
                    $product["product_code"] = $product["product_code"] = $this->_deleteHTML_Elements($product["product_code"]);
                    
                    fwrite($f, "						<code_1c>".$product["code_1c"]."</code_1c>\n");
                    
                    if (strlen($dsc) > 0) {
                        $product[$dsc] = $this->_deleteHTML_Elements($product[$dsc]);
                        fwrite($f, "						<description>".$product[$dsc]."</description>\n");
                    } else {
                        fwrite($f, "						<description></description>\n");
                    }

//                    if ($sales_notes) {
//                        fwrite($f, "						<sales_notes>" . $sales_notes . "</sales_notes>\n");
//                    } elseif ($product["min_order_amount"] > 1) {
//                        fwrite($f, "						<sales_notes>Минимальный заказ: " . $product["min_order_amount"] . " шт.</sales_notes>\n");
//                    }
//                    if (trim($product["eproduct_filename"]) != "") {
//
//                        fwrite($f, "						<downloadable>true</downloadable>\n");
//                    } else {
//                        fwrite($f, "						<downloadable>false</downloadable>\n");
//                    }
                    fwrite($f, "					</offer>\n");
                }
                db_free_result($q);
