...
//                }                
                ...
                switch ($a['ukey']) {
                    ...
                    case 'auxpage_grandtoys':
                        ...
                        break;
                    default:
                        if (detectMobile() && $a['ukey'] == 'auxpage_new_items') {
...
//                    $category = $category_name[$Product->categoryID];
                    ...
                    $shop_count = 'Купить';
                    $cart_empty_class = ' cart_empty';
                    $cart_icon_class = 'glyphicons-203-shopping-cart';
                    if (($shop_count_cart[$Product->productID])) {
                        $shop_count = $shop_count_cart[$Product->productID];
                        $cart_empty_class = '';
                        $cart_icon_class = 'glyphicons-540-cart-tick';
                    }
                    $add2cart = ($buy_enabled)?"
                                                <div class=add_to_cart>
                                                    ...
                                                    <button class='z_add_cart$cart_empty_class' onclick='add2Cart(\"#qty$Product->productID\")' type=button>
                                                        <div class=in_cart>
                                                            <span class='glyphicons $cart_icon_class' aria-hidden='true'></span>
                                                            <div id=zpid_$Product->productID>$shop_count</div>
                                                        </div>
                                                    </button>
                                                </div>
                    ":'';
                    ...
                    if ($pics_for_slider > 0) {
                        $pictures = '
                        <div class="slider visual" data-id="'.$Product->code_1c.'" data-pics="'.$pics_for_slider.'">
                            <div class=controls>
                                <div class="label prev_pic" onclick="changePic(\''.$Product->code_1c.'\',-1)"></div>
                                <div class="label next_pic" onclick="changePic(\''.$Product->code_1c.'\', 1)"></div>
                            </div>
                            <a href="/product/'.$Product->slug.'">
                                <img id=pic'.$Product->code_1c.' class=preview data-pics='.$pics_for_slider.' data-current=0 src='.URL_PRODUCTS_PICTURES.'/'.$Product->thumbnail.' data-pid='.URL_PRODUCTS_PICTURES.'/'.$Product->filename.' alt="'.$Product->name_ru.'">
                            </a>
                            '.$label.'
                        </div>';
                    } else {
                        $pictures =
                            ((strlen($Product->thumbnail) > 4)
                                &&
                                (strlen($Product->filename) > 4))
                                ?
                                "<div class=visual data-id=$Product->code_1c>
                                    <a href='/product/$Product->slug'>
                                        <img id='pic$Product->code_1c' class=preview  alt='$Product->name_ru' src='" . URL_PRODUCTS_PICTURES . "/$Product->thumbnail' data-pid='" . URL_PRODUCTS_PICTURES . "/$Product->filename'>
                                    </a>
                                    $label
                                </div>"
                                :
                                "<div class=visual data-id=$Product->code_1c>
                                    <a href='/product/$Product->slug'>
                                        <img  id='pic$Product->code_1c' alt='no foto' src='/img/nophoto.jpg'>
                                    </a>
                                    $label
                                </div>";
                    }
                ...
                $newitems_end = '</div>
                                <div id=parent_popup>
                                        <div id=popup class="animated zoomIn"  onclick="hidePopup(\'parent_popup\');">
                                            <div id=modal>
                                            ...
                                            <span class=close onclick="hidePopup(\'parent_popup\');">&times;</span>
                                        </div>
                                    </div>';
