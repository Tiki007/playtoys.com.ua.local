...
//            }
            ...
            foreach ($_GET as $get_key => $get_key) {
                ...
                switch ($get_key) {
                    ...
                    case 'bestsellers':
                        $this->__getProductsList('items_sold', 100);
                        break;
...
//                }
        ...
        protected function __setMargin()
        {
            $margin = (int)$_GET['margin'];
            $this->margin = $margin?' AND margin > '.$margin:'';
        }
        ...
        protected function __getProductsFromLists($list)
        {
            ...
            $this->$list = ' AND productID IN ('.$ids.')';
        }
...
//            $this->competitor = '';
            ...
            if (count($this->conc)) {
                if (isset($_GET['inner_join'])) {
                    $this->join = ' AND ';
                }
                $this->competitor = ' AND (' . xEscapeSQLstring(implode($this->join, $this->conc)) . ')';
            }
        }
...
//            $Grid->__debug = true;            
            ...
            $Grid->query_total_rows_num = "
                ...
                WHERE enabled = 2
                    ...
                    $this->new $this->newitemspostup $this->akcia $this->competitor $this->search
                    $this->margin";
